"""
Supervised Fine-Tuning für Fundamental Analysis Agent
...
class FundamentalSFTTrainer:
    """Trainer für Fundamental Analysis Agent"""
    ...
    def __init__(self, config_path: str):
        ...
        if self.config.get('use_wandb', True):
            wandb.init(
                ...
                name=f"fundamental-sft-{self.config['model']['base_model'].split('/')[-1]}",
                config=self.config
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Fundamental Analysis Agent")
    parser.add_argument(
        '--config',
        type=str,
        default='config/sft/fundamental_agent.yaml',
    ...
    # Train
    trainer = FundamentalSFTTrainer(args.config)
    trainer.train()
